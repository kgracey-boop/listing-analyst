...
FIELD_ALIASES = {
    ...
    "concessions_amount": ["concessions amount", "concessions", "seller concessions", "concession amount"],
    "concessions_comments": ["concessions comments", "concession comments", "concessions comment"],
...
def parse_mls_csv(file_obj) -> dict:
    ...
    for row in reader:
        listing = {
            ...
            "concessions_amount": _parse_number(row.get(column_map.get("concessions_amount"))),
            "concessions_comments": (row.get(column_map.get("concessions_comments"), "") or "").strip() or None,
